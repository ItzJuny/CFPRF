...
def pad_sequence(batch, padding_value=0.0):
    ...
    # print("sadsadlksajds",max_size,trailing_dims,max_len)
    if all(x.shape[0] == max_len for x in batch):
        ...
        # print("out_dims",out_dims,max_len,trailing_dims)
        output_batch = []
        for i, tensor in enumerate(batch):
            ...
            # print(i,"max_size",max_size,"trailing_dims",trailing_dims,"max_len",max_len,"out_dims",out_dims,"out_tensor.size()",out_tensor.size(),"tensor.size(0)",tensor.size(0))
        return output_batch
...
def customize_collate(batch):
    ...
    if isinstance(elem, torch.Tensor):
        ...
        #     print(len(batch[i]),len(batch_new[i]))
        out = None
        ...
        # print("56656565656",torch.stack(batch).size())
